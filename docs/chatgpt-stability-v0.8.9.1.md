...
## 后发同版本重发授权与源码闭环（2026-09-08）

用户已明确授权保留版本号0.8.9.1，验证后替换该tag/Release及本机运行包；其它历史版本不改。上述初次发布和追加FAIL保留为历史证据，不能理解为这次新增修复已经安装。

新增同步工具有界worker、取消等待隔离、进程级4名额、执行前权限复查、实际错误审计、默认Shell单预算探测已实现。原4项并发反例保持原文件哈希，4 failed→4 passed；真实子进程可回调同一HTTP Hub。最终源码Python604 passed/3 skipped，Ruff/双平台Pyright/TS build及完整smoke通过。smoke曾因项目内TEMP让nonGit fixture继承父Git仓库而失败；只修正runner隔离，不删除断言或改变nested-Git行为。

旧版全部六资产、元数据、tag对象与源码归档已在release/rollback-v0.8.9.1-08ef5a8核验保存，远端immutable=false。新Windows/Linux仍须在同一源提交构建并验证，之后先上传候选并校验再替换正式名称，不使用先删后传且无回滚副本的clobber。最终新源提交、CI、资产SHA与实际安装证明以release-provenance.json及进度验收后续节为准。版本相同不会凭语义版本自动触发其它电脑更新；本机采用本轮明确安装。

本次没有将原截图唯一归因于MCP，也没有宣称所有长期内存/输出捕获风险已消失；存量dist-info及极大输出的流式捕获不纳入同版本窄重发。

## 同版本重发最终运行态（2026-09-08）

版本仍0.8.9.1，线上与本机均已替换为53ed46978a1f67da4ddc8e0b0ba74277dc805dcc同源正式包，CI34199524211：Windows613/3、Linux605/11（passed/skipped）。本机16:03:09+08:00完成原目录安装，实际冻结EXE哈希16aede49ff48517f4ec5a60397780e004412d78fda832a560e1238c9e3c9feec与CI一致。五根恢复、鉴权正负向、真实管理员子进程回调同Hub与并发health31ms通过。冷/热search约15.2秒返回20命中，structured超预算警告仍明确保留。正式6资产、回滚副本和详细证据见进度验收§0.6；早期源及未完成描述只代表历史检查点，未改写失败证据。
