...
Output is kept in a bounded rolling buffer, but the MCP transport projection is smaller than the internal task record: running task stdout/stderr use roughly 2 KiB UTF-8-safe tails per stream, terminal task detail uses at most about 8 KiB per stream plus omitted-byte counters, and commands are capped to about 2 KiB. `list_tasks` is an active-first compact index of at most 20 summaries and never bulk-returns stdout/stderr. When compact task output omitted bytes that are still retained internally, the existing stable `codexpro(action,args)` wrapper exposes read-only `action=task_output`: pages default to 8 KiB and cap at 16 KiB, preserve UTF-8 boundaries, report retained/observed byte ranges and whether earlier rolling output is permanently unavailable, and use a process-local HMAC-sealed opaque cursor bound to task/stream/revision/offset. This adds no registered MCP tool and does not increase the rolling-buffer retention ceiling. Cancellation terminates the process tree. A 600-second orchestration watchdog only annotates a stale task snapshot with a resume hint; it never terminates the task. Running tasks are not resumed across a DevBridge/CodexPro process restart.
...
The state file `.ai-bridge/long-runs/<run_id>.json` is schema-bounded, atomically replaced, path-guarded, serialized per run inside the process, and rejects secret-looking persisted text. It remains the complete durable source of truth. MCP long-run results use a separate bounded transport summary: counts plus recent task IDs/resolutions, checkpoint tails and UTF-8 byte-bounded criteria/evidence/notes, so repeated status/update/review calls do not reinject the complete durable history into a host conversation. When older durable history is required for the current decision, the same stable `codexpro(action,args)` wrapper exposes read-only `action=run_detail` for cursor-paged `checkpoints`, `task_resolutions`, or one step's `acceptance_criteria/evidence/notes`. Cursors are HMAC-sealed and revision-bound, so cross-run/tampered/stale cursors fail closed instead of mixing snapshots. `workRevision` changes whenever meaningful work changes; a PASS review is only valid for the revision it inspected. A later mutation makes that PASS stale. FAIL reviews require explicit failed criteria/rework and reopen affected steps.

Attached tasks gate review/completion. Running/cancelling work blocks PASS. After a CodexPro process restart an old task id may be unknown because command processes are intentionally not resumed; completion then fails closed until an explicit terminal resolution with evidence is persisted. This avoids interpreting lost process state as success. Planned project-engine hot reloads therefore drain `running/cancelling` tasks across all opened workspaces for that engine before restart; a busy root is deferred rather than process-tree-killed as an upgrade side effect.

MCP polling remains bounded but capability-sensitive: without progress-notification support one `wait_task` is capped at 15 seconds; with a request `progressToken` it may wait up to 120 seconds and emits standard progress about every 8 seconds. The shorter no-progress window limits one host-silent status poll only and never cancels the background task. A `wait_task` status poll is also the only `tools/call` that may receive one internal retry when the Gateway loses the upstream connection before response headers, because it is read-only and cannot duplicate the underlying command; mutating tools remain single-attempt. Running poll payloads carry only small output tails, while adaptive hints stretch from 5 to 120 seconds as task age grows. The Gateway also emits 12-second comment keepalives on otherwise-idle SSE streams only at complete event boundaries. CodexPro HTTP now uses MCP TypeScript SDK v2 `createMcpHandler`: modern 2026-07-28 requests are per-request stateless and legacy 2025 traffic uses the SDK's stateless fallback, so there is no protocol-session TTL or session transport map. Workspace handles, background tasks and durable long-runs live in process-level application state; client-specific selection/checkpoint state is bounded and keyed by a non-authoritative client-affinity hash. Native `io.modelcontextprotocol/tasks` remains a future capability-negotiated mapping. See `LONG_RUNNING_TASKS.md` and `GOAL_LONG_RUNNING_TASKS.md`.
