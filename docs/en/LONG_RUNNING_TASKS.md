...
- `wait_task`: bounded polling request. Without a request progress token one call is capped at 15 seconds. When the client supplies an MCP `progressToken` and the SDK exposes notification delivery, the request may wait up to 120 seconds and emits `notifications/progress` about every 8 seconds. Reaching either deadline never stops the background process. The Gateway may retry this read-only status poll once if its upstream connection fails before response headers; mutating tool calls remain single-attempt because their outcome after disconnect may be unknown.
