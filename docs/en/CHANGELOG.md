...
- Harden the optional Windows control lifecycle after a real `uvx` orphan/restart storm: Windows-MCP stop/failure paths reclaim only the manager-owned live process tree, refuse stale-PID tree termination after the launcher has already exited, and serialize start/stop/readiness by process generation. Project READY/routing and CodexPro health recovery are now independent from the optional Windows bridge, so a bridge startup/error condition degrades only Windows control rather than repeatedly restarting a healthy project. Explicit project stop still closes both components. This maintenance-source repair is validated but must not be described as deployed to a machine until that machine completes its own upgrade and runtime smoke.
...
- Give lexical and structured repository search one 20-second response budget, incremental partial results, request cancellation and eight actual in-flight operation slots. Keep pending I/O counted after timeout; bound ripgrep output and clean up owned child processes. Preserve fixed tool schemas and explicitly report incomplete scans. Isolate every pytest process from production configuration/log directories before product imports.
